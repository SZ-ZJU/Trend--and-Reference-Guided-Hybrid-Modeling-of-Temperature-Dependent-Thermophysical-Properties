...
v_cols = df.columns[41:51]      # 第36~45列，火用
...
df_result = pd.DataFrame({
    ...
    "Exergy_measured (J/mol)": y_total,  # 根据文件名改为Exergy
    "Exergy_predicted (J/mol)": y_pred,   # 根据文件名改为Exergy
    "Absolute Error": np.abs(y_total - y_pred),
    "Relative Error (%)": relative_error
})

df_result.to_excel("Exergy预测结果_基团加温度_GBR.xlsx", index=False)
print("✅ 已保存预测结果为: Exergy预测结果_基团加温度_GBR.xlsx")