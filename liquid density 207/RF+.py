...
# ==== 2. Hvap 模型（298 K） ====
df_298 = pd.read_excel("selected_25_descriptors_normal.xlsx", sheet_name="Sheet1")
...
results.to_excel("Density预测结果_加slope特征_RF.xlsx", index=False)
print("✅ 已保存预测结果为：Density预测结果_加slope特征_RF.xlsx")
...
print("\n📊 各子模型精度：")
print(f"🔥 Dsity@298K  — R² = {r2_298:.4f}, MSE = {mse_298:.8f}")
print(f"🔥 Dsity@Tb    — R² = {r2_Tb:.4f}, MSE = {mse_Tb:.8f}")
print(f"🌡️  Tb预测     — R² = {r2_Tb_pred:.4f}, MSE = {mse_Tb_pred:.8f}")

hvap_compare = pd.DataFrame({
    "Dsity_298_True": y_298,
    "Dsity_298_Pred": HVap_298_all,
    "Dsity_Tb_True": y_Tb,
    "Dsity_Tb_Pred": HVap_Tb_all,
    "Dsity_True": Tb_true,
    "Dsity_Pred": Tb_pred
})
hvap_compare.to_excel("Dsity_Tb_模型精度对比.xlsx", index=False)
print("✅ 已保存模型精度对比文件为：Dsity_Tb_模型精度对比.xlsx")
