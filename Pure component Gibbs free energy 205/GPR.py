...
v_cols = df.columns[41:51]      # 第36~45列，吉布斯自由能
...
df_result = pd.DataFrame({
    ...
    "Gibbs_free_energy_measured (J/mol)": y_total,  # 根据文件名改为吉布斯自由能
    "Gibbs_free_energy_predicted (J/mol)": y_pred,   # 根据文件名改为吉布斯自由能
    "Absolute Error": np.abs(y_total - y_pred),
    "Relative Error (%)": relative_error
})

df_result.to_excel("Gibbs_free_energy预测结果_基团加温度_GBR.xlsx", index=False)
print("✅ 已保存预测结果为: Gibbs_free_energy预测结果_基团加温度_GBR.xlsx")