...
v_cols = df.columns[42:52]      # 第36~45列，体积
...
df_result = pd.DataFrame({
    ...
    "Volume_measured (m³/mol)": y_total,  # 根据文件名改为体积，添加适当单位
    "Volume_predicted (m³/mol)": y_pred,   # 根据文件名改为体积，添加适当单位
    "Absolute Error": np.abs(y_total - y_pred),
    "Relative Error (%)": relative_error
})

df_result.to_excel("Volume预测结果_基团加温度_GBR.xlsx", index=False)
print("✅ 已保存预测结果为: Volume预测结果_基团加温度_GBR.xlsx")