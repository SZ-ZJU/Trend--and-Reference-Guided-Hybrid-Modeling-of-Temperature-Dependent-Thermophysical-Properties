...
v_cols = df.columns[42:52]       # 第43~52列：体积
...
for i in range(len(df)):  # 遍历所有物质
    for j, (tcol, vcol) in enumerate(zip(temp_cols, v_cols)):
        ...
        HVap_Tb_i = HVap_Tb_all[i]  # 物质i的参考体积值
...
print(f"MSE (on Volume): {mse_v:.6f}")
print(f"R2  (on Volume): {r2_v:.6f}")
...
out_path = "volume_actual_vs_pred_long.xlsx"
